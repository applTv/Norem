...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="container">
            <form method="POST" action="{{route('hrono.update', $hrono)}}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            <div class="mb-3">
                <label for="name" class="form-label">Фракция в которой произошло событие</label>
                <textarea name="name"class="form-control" id="name" rows="1">{{$hrono-> name}}</textarea>
              </div>
              <div class="mb-3">
                <label for="year_event" class="form-label">Год когда произошло событие</label>
                <textarea name="year_event"class="form-control" id="year_event" rows="1">{{$hrono-> year_event}}</textarea>
              </div>
              <div class="mb-3">
                <label for="title" class="form-label">Описание события</label>
                <textarea name="title"class="form-control" id="title" rows="5">{{$hrono-> title}}</textarea>
