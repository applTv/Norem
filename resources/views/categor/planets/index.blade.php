...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div class="container">
        ...
        <h2 class="p-3 text-center">Планеты</h2>
        <div class="row">
            ...
            <div class="col-lg-2 col-md-4 mb-4 mb-md-0 imgdiv">
              ...
              @if(Auth::user()->isAdmin())
                  ...
                  <a href="{{ route('planets.create') }}" class="btn btn-danger m-3">Добавить планету</a>
              @endif
      @endauth
      <div class="list-group list-group-light p-3">
          @foreach ($planets as $planet)
              <a href="{{ route('planets.show' , $planet->id) }}" class="list-group-item list-group-item-action px-2 border-0">
                <h5>{{$planet-> planets_name}}</h5></a>
          @auth
              @if(Auth::user()->isAdmin())
                  <a href="{{ route('planets.edit', $planet) }}" class="btn-outline-danger m-3 p-1" role="button">Изменить</a>
                  <form method="POST" action="{{route('planets.destroy', $planet)}}">
                      @csrf
                      @method('DELETE')
                      <button type='submit'class="btn-outline-danger m-2 p-2">Удалить</button>
                    </form>
              @endif
          @endauth
          @endforeach
