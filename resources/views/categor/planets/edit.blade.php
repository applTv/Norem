...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="container">
            <form method="POST" action="{{route('planets.update',$planet)}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
            <div class="mb-3">
                <label for="planets_name" class="form-label">Название палнеты:</label>
                <textarea name="planets_name"class="form-control" id="planets_name" rows="1">{{$planet-> planets_name}}</textarea>
              </div>
              <div class="mb-3">
                <label for="climate" class="form-label">Климат:</label>
                <textarea name="climate"class="form-control" id="climate" rows="3">{{$planet-> climate}}</textarea>
              </div>
              <div class="mb-3">
                <label for="description" class="form-label">Описание планеты:</label>
                <textarea name="description"class="form-control" id="description" rows="5">{{$planet-> description}}</textarea>
              </div>
              <div class="mb-3">
                <label for="population" class="form-label">Население:</label>
                <textarea name="population"class="form-control" id="population" rows="4">{{$planet-> population}}</textarea>
              </div>
              <div class="mb-3">
                <label for="time" class="form-label">Количество дней и часов в сутках: </label>
                <textarea name="time"class="form-control" id="time" rows="2">{{$planet-> time}}</textarea>
              </div>
              <div class="mb-3">
                <label for="images" class="form-label">Изображение Планеты</label>
                <input name="images"class="form-control" type="file" rows="1">
              </div>
