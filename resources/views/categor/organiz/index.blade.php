...
<div class="container">
    ...
    <h2 class="p-3 text-center"> Организации</h2>
    @if(session()->has('message'))
      ...
      @endif
    <div class="row">
        <div class="col-lg-5 col-md-4 mb-5 mb-md-0 imgdiv"></div>
        <div class="col-lg-3 col-md-4 mb-3 mb-md-0 imgdiv">
          @auth
                    @if(Auth::user()->isAdmin())
                      <a href="{{ route('organiz.create') }}" class="btn btn-danger m-3">Добавить Организацию</a>       
                    @endif
              @endauth
    <div class="list-group list-group-light">
              @foreach ($organizs as $organiz)
              <a href="{{ route('organiz.show' , $organiz->id) }}" class="list-group-item list-group-item-action px-2 border-0">
                <h5 class="">{{$organiz-> Organiz_name}}</h5></a>
          @auth
              @if(Auth::user()->isAdmin())
              <div class="d-grid gap-2 d-md-block">
                  <a href="{{ route('organiz.edit', $organiz) }}" 
                    class="btn-outline-danger btn-sm m-2 me-md-2" role="button">Изменить</a>
                  
                  <form method="POST" action="{{route('organiz.destroy', $organiz)}}">
                      @csrf
                      @method('DELETE')
                      <button type='submit'class="btn-outline-danger btn-sm m-2">Удалить</button>
                    </form>
                  </div>
              @endif
          @endauth
          <form action="{{url('/add_organiz',$organiz->id)}}" method="POST">
            @csrf
          <input class="btn-outline-warning btn-sm m-2 me-md-2" type="submit" value="В избранное">
          </form>
          @endforeach
      </div>
      {{-- {{$organizs -> links()}} --}}
    </div>
      <div class="col-lg-4 col-md-4 mb-4 mb-md-0 imgdiv"></div>
    </div>
</div>
