...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div class="container">
        ...
        <h2 class="p-3 text-center">Магия</h2>
        <div class="row">
            <div class="col-lg-5 col-md-4 mb-4 mb-md-0 imgdiv"></div>
            <div class="col-lg-3 col-md-4 mb-4 mb-md-0 imgdiv">
              @auth
                  @if(Auth::user()->isAdmin())
                    <a href="{{ route('magic.create') }}" class="btn btn-danger m-3">Добавить болезнь</a>       
                  @endif
               @endauth
            <div class="list-group list-group-light">
                @foreach ($magics as $magic)
                  <a href="{{ route('magic.show' , $magic->id) }}" class="list-group-item list-group-item-action px-3 border-0">
                    <h5>{{$magic-> magic_name}}</h5></a>
                   @auth
                      @if(Auth::user()->isAdmin())
                      <div class="d-grid gap-2 d-md-block">
                            <a href="{{ route('magic.edit', $magic) }}" class="btn-outline-danger btn-sm m-2 me-md-2" role="button">Изменить</a>
                            <form method="POST" action="{{route('magic.destroy', $magic)}}">
                              @csrf
                              @method('DELETE')
