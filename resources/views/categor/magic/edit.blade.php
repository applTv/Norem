...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <body>
        ...
        <div class="container">
            <form method="POST" action="{{route('magic.update', $magic )}}" enctype="multipart/form-data">
                @csrf
                @method('PUT')
              <div class="mb-3">
                  <label for="magic_name" class="form-label">Изменить название магии</label>
                  <textarea name="magic_name"class="form-control" id="magic_name" rows="1">{{$magic-> magic_name}}</textarea>
                </div>
                <div class="mb-3">
                  <label for="legality" class="form-label">Изменить легальность магии</label>
                  <textarea name="legality"class="form-control" id="legality" rows="1">{{$magic-> legality}}</textarea>
                </div>
                <div class="mb-3">
                  <label for="restrictions" class="form-label">Изменить кто может использовать магию</label>
                  <textarea name="restrictions"class="form-control" id="restrictions" rows="1">{{$magic-> restrictions}}</textarea>
                </div>
                <div class="mb-3">
                  <label for="magic_description" class="form-label">Изменить для чего используется</label>
                  <textarea name="magic_description"class="form-control" id="magic_description" rows="5">{{$magic-> magic_description}}</textarea>
                </div>
                <div class="mb-3">
                  <label for="add_description" class="form-label">Изменить дополнительную информацию</label>
                  <textarea name="add_description"class="form-control" id="add_description" rows="5">{{$magic-> add_description}}</textarea>
                </div>
