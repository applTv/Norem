...
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}">
    ...
    <div class="container">
        ...
        <h2 class="p-3 text-center"> Персонажи</h2>
        ...
        <div class="row">
            ...
            <div class="col-lg-3 col-md-4 mb-4 mb-md-0 imgdiv">
              @auth
                    @if(Auth::user()->isAdmin())   
                        <a href="{{ route('character.create') }}" class="btn btn-danger m-3">Добавить персонажа</a>
                    @endif
                @endauth
        <div class="list-group list-group-light">
            @foreach ($characters as $character)
                <a href="{{ route('character.show' , $character->id) }}" 
                  ...
                  <h5>{{$character-> character_name}}</h5></a>
                  @auth
              @if(Auth::user()->isAdmin())
                <div class="d-grid gap-2 d-md-block">
                  <a href="{{ route('character.edit', $character) }}" class="btn-outline-danger btn-sm m-2 me-md-2" role="button">Изменить</a>
                  <form method="POST" action="{{route('character.destroy', $character)}}">
                      @csrf
                      @method('DELETE')
              ...
              @endif
          @endauth
            <form action="{{url('/add_character',$character->id)}}" method="POST">
              @csrf
